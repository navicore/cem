...
// Forward declare close() to avoid including unistd.h (which conflicts with
// stack.h's dup())
 ...
 * This ensures proper cleanup ordering (e.g., inner resources freed before
 * outer).
 */
static void strand_run_cleanup_handlers(Strand *strand) {
  ...
  while (handler) {
    ...
    if (handler->func) {
      ...
      // This should never happen - it indicates a bug in cleanup handler
      // registration
 ...
 * Useful for realloc operations where the pointer changes but cleanup remains
 * the same.
 */
...
void strand_block_on_read(int fd) {
  ...
  ev.events =
      EPOLLIN | EPOLLET | EPOLLONESHOT; // Edge-triggered, one-shot like kqueue
  ev.data.ptr = strand;
...
void strand_block_on_write(int fd) {
  ...
  struct epoll_event ev;
  ev.events =
      EPOLLOUT | EPOLLET | EPOLLONESHOT; // Edge-triggered, one-shot like kqueue
  ev.data.ptr = strand;
  if (epoll_ctl(global_scheduler.epoll_fd, EPOLL_CTL_ADD, fd, &ev) == -1) {
...
StackCell *scheduler_run(void) {
  ...
  // When strands yield, they save their context and restore the scheduler
  // context The scheduler context gets populated on the first swap
  ...
  while (true) {
    ...
    if (!ready_queue_is_empty()) {
      ...
      fprintf(stderr, "[SCHED] Returned from swapcontext\n");
      fflush(stderr);
      ...
      if (strand->state == STRAND_COMPLETED) {
        ...
        // Nothing to do here, just continue to next iteration
                 ...
                 strand->state == STRAND_BLOCKED_WRITE) {
        // Strand blocked on I/O - it already added itself to blocked list and
        // registered with kqueue Nothing to do here, continue to next iteration
      } else {
        // Unexpected state
        runtime_error(
      ...
      for (int i = 0; i < nevents; i++) {
        ...
        // Remove from epoll (one-shot behavior - already removed automatically
        // by EPOLLONESHOT) No need to call epoll_ctl with EPOLL_CTL_DEL -
        // EPOLLONESHOT does it for us
